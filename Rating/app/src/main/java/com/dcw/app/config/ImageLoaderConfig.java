...
public class ImageLoaderConfig {
    ...
    private static ImagePipelineConfig configureCaches(Context context) {
        ...
        final MemoryCacheParams bitmapCacheParams = new MemoryCacheParams(
                ImageLoaderConfig.MAX_MEMORY_CACHE_SIZE, // 内存缓存中总图片的最大大小,以字节为单位。
                Integer.MAX_VALUE,                     // 内存缓存中图片的最大数量。
                ImageLoaderConfig.MAX_MEMORY_CACHE_SIZE, // 内存缓存中准备清除但尚未被删除的总图片的最大大小,以字节为单位。
...
//            .setDiskTrimmableRegistry(diskTrimmableRegistry)//类将包含一个注册表的缓存减少磁盘空间的环境。
                .setMaxCacheSize(ImageLoaderConfig.MAX_DISK_CACHE_SIZE)//默认缓存的最大大小。
...
//            .setFadeDuration(fadeDuration)//fresco:fadeDuration="300"加载图片动画时间
                .setFailureImage(ImageLoaderConfig.sErrorDrawable)//fresco:failureImage="@drawable/error"失败图
...
//            .setOverlays(overlays)
                .setPlaceholderImage(ImageLoaderConfig.sPlaceholderDrawable)//fresco:placeholderImage="@color/wait_color"占位图
...
//            .setLowResImageRequest(ImageRequest.fromUri(lowResUri))//先下载显示低分辨率的图
    ...
    public static void init(final Resources resources) {
        if (sPlaceholderDrawable == null) {
            sPlaceholderDrawable = resources.getDrawable(android.R.color.transparent);
        }
        if (sErrorDrawable == null) {
            sErrorDrawable = resources.getDrawable(android.R.color.white);
        }
    }
}